...
function findSubtypes (ts, node, code, types) {
  ...
  types?.forEach((type) => {
    ...
    st?.members?.forEach((member) => {
      ...
      const isUnion = member.type.kind === ts.SyntaxKind.UnionType;
      if (isUnion) {
        member.type.types.forEach((type) => {
          ...
          if (customType != null) {
            formattedTypes.push(customType);
            formattedTypes.push(...findSubtypes(ts, node, code, [customType]));
          }
        });
      }
      else if (member.type.kind === ts.SyntaxKind.TypeReference || member.type?.elementType?.kind === ts.SyntaxKind.TypeReference) {
