...
import { objectEquals } from '../src/lib/utils.js';
import {
  combineLatest,
  ...
  return `
    ...
    <div class="list">
      ${(context.ownerId != null) ? `
        <ul>
          ${(summary.organisations.find(({ id }) => id === context.ownerId) || { applications: [] }).applications.map((a) => `
            <li><a href="#${context.ownerId}/${a.id}">${a.name}</a></li>
          `).join('')}
          ${(summary.user.id === context.ownerId) ? summary.user.applications.map((a) => `
            <li><a href="#${context.ownerId}/${a.id}">${a.name}</a></li>
          `).join('') : ''}
        </ul>
      ` : ''}
