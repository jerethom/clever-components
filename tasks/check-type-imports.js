...
function checkTypeImports (componentName, filepath, sourceCode) {
  ...
  const imports = { component: componentName, filepath: filepath, unusedImports: [], errors: [] };
  classNode?.jsDoc?.forEach((jsDoc) => {
    jsDoc.tags
      ...
      .forEach((tag) => {
        ...
        if (typeDefDisplay == null) {
          imports.errors.push(tag.getText());
        }
...
function run () {
  ...
  unusedImports.forEach((unusedImport) => {
    console.log('🔍', unusedImport.component, '-', unusedImport.filepath, '\n');
    unusedImport.errors.forEach((error) => console.log(chalk.red(`❌ Something might be wrong for @typedef: `), error));
    console.log(chalk.hex('#c15807')(`\n ⚠️  Unused import(s) are: `));
