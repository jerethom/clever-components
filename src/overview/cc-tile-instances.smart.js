...
import { getStatus, STATUSES } from '@clevercloud/client/esm/utils/app-status.js';
...
export function formatInstances (status, rawInstances) {
  const runningInstances = ([STATUSES.STOPPED, STATUSES.START_FAILED].includes(status))
    ? []
    : rawInstances
      .filter((i) => i.state === 'UP')
  ...
  const deployingInstances = ([STATUSES.STOPPED, STATUSES.START_FAILED, STATUSES.RUNNING, STATUSES.RESTART_FAILED].includes(status))
    ? []
    : rawInstances
