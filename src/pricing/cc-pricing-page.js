...
export class CcPricingPage extends LitElement {
  ...
  _filterPrice (products) {
    return products.map((p) => {
      ...
      const priceList = this.pricingLists.find((pl) => pl.zone_id === this._zone);
  ...
  _onAddProduct ({ detail: product }) {
    ...
    const id = (product.item.id != null)
      ? product.item.id
      : `${product.productName}/${product.item.name}`;
  ...
  render () {
    ...
    return html`
      ...
      <div class="runtimes">
        ${this._renderProducts('runtime')}
      </div>
      <div class="title">Addons</div>
      <div class="addons">
        ${this._renderProducts('addon')}         
      </div>
